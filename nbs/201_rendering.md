...
def make_bsdf(color=(0.85, 0.05, 0.05), ior=1.53, alpha=0.3):
    return {
        "type": "roughplastic",
        "distribution": "ggx",
        "alpha": alpha,
...
# Material-appropriate BSDFs
def bsdf_for_material(material: str, color: tuple = (0.85, 0.05, 0.05)) -> dict:
    if material == "transparent":
        return {
            "type": "dielectric",
            "int_ior": 1.53,
            "specular_transmittance": {"type": "rgb", "value": list(color)},
        }
    if material == "chrome":
        return {
            "type": "conductor",
            "specular_reflectance": {"type": "rgb", "value": [0.8, 0.8, 0.85]},
        }
    return {
        "type": "plastic",
        "diffuse_reflectance": {"type": "rgb", "value": list(color)},
...
def load_part(part_id: str) -> tuple[np.ndarray, np.ndarray]:
    scene = ldr.load_file(
        f"{LDRAW_DIR}/parts/{part_id}.dat", str(LDRAW_DIR), [], settings
    )
    key = list(scene.geometry_cache.keys())[0]
    geom = scene.geometry_cache[key]
...
def make_mesh(vertices, faces, material):
    mesh = mi.Mesh("brick", vertex_count=len(vertices), face_count=len(faces))
    params = mi.traverse(mesh)
    params["vertex_positions"] = vertices.ravel()
    params["faces"] = faces.ravel()
    params.update()
    mesh.set_bsdf(mi.load_dict(bsdf_for_material(material)))
    return mesh


fig, axes = plt.subplots(
    len(test_parts), len(spps), figsize=(5 * len(spps), 5 * len(test_parts))
)

for row, (part_id, material) in enumerate(test_parts):
    vertices, faces = load_part(part_id)
    mesh = make_mesh(vertices, faces, material)
    for col, spp in enumerate(spps):
        t0 = time.perf_counter()
        scene = make_scene(mesh, origin=(100, 50, 100), spp=spp)
        image = mi.render(scene)
        elapsed = time.perf_counter() - t0

        axes[row, col].imshow(mi.util.convert_to_bitmap(image))
        axes[row, col].set_axis_off()
        axes[row, col].set_title(f"{part_id} ({material})\nSPP {spp} — {elapsed:.2f}s")

plt.suptitle("SPP vs quality by material type", fontsize=14)
plt.tight_layout()
plt.show()
```

```python
hdri_path = list(Path("../data/hdri").glob("*.exr"))[0]
print(f"Using HDRI: {hdri_path.name}")


def make_scene_envmap(mesh, origin, hdri_path, spp=128, fov=35, resolution=512):
    return mi.load_dict(
        {
            "type": "scene",
...
spps = [128, 256, 512]
vertices, faces = load_part("3001")
mesh = make_mesh(vertices, faces, "chrome")

fig, axes = plt.subplots(1, len(spps), figsize=(5 * len(spps), 5))
for ax, spp in zip(axes, spps):
    t0 = time.perf_counter()
    scene = make_scene_envmap(mesh, origin=(100, 50, 100), hdri_path=hdri_path, spp=spp)
    image = mi.render(scene)
    elapsed = time.perf_counter() - t0

    ax.imshow(mi.util.convert_to_bitmap(image))
    ax.set_axis_off()
    ax.set_title(f"SPP {spp} — {elapsed:.2f}s")

plt.suptitle("3001 chrome — HDRI environment", fontsize=14)
plt.tight_layout()
plt.show()
```
...
fig, axes = plt.subplots(
    len(chrome_parts), len(spps), figsize=(5 * len(spps), 5 * len(chrome_parts))
)

for row, (part_id, material) in enumerate(chrome_parts):
    vertices, faces = load_part(part_id)
    mesh = make_mesh(vertices, faces, material)
    for col, spp in enumerate(spps):
        t0 = time.perf_counter()
        scene = make_scene_envmap(
            mesh, origin=(100, 50, 100), hdri_path=hdri_path, spp=spp
        )
        image = mi.render(scene)
        elapsed = time.perf_counter() - t0
...
plt.suptitle("HDRI environment — SPP convergence by material", fontsize=14)
plt.tight_layout()
plt.show()
```
